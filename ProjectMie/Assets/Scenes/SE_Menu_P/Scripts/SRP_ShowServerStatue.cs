...
public class SRP_ShowServerStatue : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if(StatueImage.Count != 5) Destroy(this);//必须四张图片，写死的
    }
    ...
    void UpdateImage(float PingInSecond)
    {
        ...
        if(PingInSecond <= 1.0f && PingInSecond > 0.3f) IMAGE.sprite = StatueImage[2];
        if(PingInSecond <= 3.0f && PingInSecond > 1f) IMAGE.sprite = StatueImage[3];
        if(PingInSecond > 3.0f) IMAGE.sprite = StatueImage[4];
    }
}
