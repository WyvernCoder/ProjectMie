...
public class SRP_guo4_fish : MonoBehaviour
{
    private Rigidbody2D PhysX;
    public uint wordIndex = 0;
    public float jumpForce = 200f;
    public float dragForce = 100f;
    private bool isMouseDown = false;
    void Start()
    {
        PhysX = GetComponent<Rigidbody2D>();
        StartCoroutine(Fish());
    }
    ...
    IEnumerator Fish()
    {
        while(true)
        {
            PhysX.AddForce(new Vector2(jumpForce * UnityEngine.Random.Range(-0.5f, 0.5f), jumpForce * UnityEngine.Random.Range(0.5f, 1f) ));
            yield return new WaitForSeconds( UnityEngine.Random.Range(0f, 3f) );
        }
    }

    public void CONTROL_MDown()
    {
        PhysX.gravityScale = 0f;
        PhysX.drag = 3f;
        isMouseDown = true;
    }

    public void CONTROL_MUp()
    {
        PhysX.gravityScale = 1f;
        PhysX.drag = 0f;
        isMouseDown = false;
    }

    private Vector3 mouse3DPos = Vector3.zero;
    private Vector3 force = Vector3.zero;
    void FixedUpdate()
    {
        if(isMouseDown)
        {
            mouse3DPos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
            force = Math_Normalize(mouse3DPos - transform.position);

            PhysX.AddForceAtPosition(force * dragForce, mouse3DPos);
        }
    }
