...
public class SRP_LineDrawerWithMission : MonoBehaviour
{
    ...
    [Tooltip("完成全部任务后是否返回主菜单")]
    public bool canBackToMenu = false;
    [Tooltip("背景图片组件")]
    public SpriteRenderer BGImage;
    [Tooltip("接下来的任务背景图是否也要使用第一个任务的背景图？")]
    public bool isIndependBGImage = false;
    ...
    IEnumerator MissionComplete()
    {
        ...
        if (NextLineDrawer != null) 
        {
            if(isIndependBGImage == false) Instantiate(NextLineDrawer).BGImage.sprite = BGImage.sprite;
            else Instantiate(NextLineDrawer);
        }
        else if(canBackToMenu == true) APIManager.API.API_WinTheGame();
        Destroy(gameObject);
        yield break;
    }
}
