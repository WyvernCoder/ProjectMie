...
public class SRP_LoadingScene : MonoBehaviour
{
    ...
    IEnumerator GoLoading_IE()
    {
        ...
        {
            Debug.LogAssertion("游戏载入必须依赖APIManager。");
            SweepTrash();//清理自己
            yield break;
        }
        ...
        else
        {
            ...
            if (!File.Exists(savePath + "/" + APIManager.API.API_LoadScene_GetName() + ".ab"))
            {
                ...
                //定义一个WebRequest下载请求
                var WebRequest = UnityWebRequest.Get(APIManager.API.HttpServer + "/OnlineAssets/" + APIManager.API.API_LoadScene_GetName() + ".ab");
                ...
                {
                    ProcessBar.value = Mathf.Clamp(Mathf.Lerp(ProcessBar.value, WTool.Selector_<float>(1f, WebRequest.downloadProgress, WebRequest.isDone), 0.01f), 0f, 1f);
                ...
                if (filebytes.Length <= 1024)//如果文件小于1MB
                {
                    Debug.LogError("没有下载任何文件，返回主菜单。");
                    //APIManager.API.API_BackToMenu();  这里用这个会死循环
                    APIManager.API.SceneHistory.Add("SE_Selector");
                    SceneManager.LoadScene("SE_Selector");
                    SweepTrash();//善后工作
                    yield break;
                }
            ...
            if (!Application.CanStreamedLevelBeLoaded(APIManager.API.API_LoadScene_GetName()))
            {
                Debug.LogWarning("无法从AB包中加载" + APIManager.API.API_LoadScene_GetName() + "关卡，准备读取AB包中的第一个关卡。");

                if (AB.GetAllScenePaths().Length == 0)
                {
                    Debug.LogError("你这AB包不保熟啊，里面压根就没有关卡，准备返回主菜单。");
                ...
                if (!Application.CanStreamedLevelBeLoaded(APIManager.API.API_LoadScene_GetName()))
                {
                    Debug.LogError("关卡仍然无法被加载，出现未知错误，准备返回主菜单。");
                    SweepTrash(true);//善后工作
                    APIManager.API.API_BackToMenu();
                    yield break;
                }
